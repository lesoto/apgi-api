...
def _make_sim_session(state_val="running"):
    s = MagicMock()
    s.session_id = "sess-1"
    s.state = MagicMock()
...
def _make_task_query_chain(tasks):
...
@pytest.fixture
def client(mock_db, mock_manager, mock_redis):
...
class TestDependencyErrors:
    ...
    def test_get_redis_client_initialized(self):
    ...
    def test_get_session_manager_initialized(self):
...
class TestSessionRoutesValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_session_ownership_success(self, mock_db, mock_manager):
    ...
    @pytest.mark.asyncio
    async def test_validate_session_ownership_not_found(self, mock_db, mock_manager):
    ...
    @pytest.mark.asyncio
    async def test_validate_session_ownership_access_denied(self, mock_db, mock_manager):
        session = Mock(spec=SessionModel)
        session.session_id = "session123"
    ...
    @pytest.mark.asyncio
    async def test_validate_session_ownership_admin_bypass(self, mock_db, mock_manager):
        session = Mock(spec=SessionModel)
        session.session_id = "session123"
        session.user_id = "other_user"
        session.is_deleted = False
...
class TestSessionRoutesInit:
    def test_init_session_routes(self):
        mock_redis = Mock()
...
class TestListSessionsUnit:
    ...
    @pytest.mark.asyncio
    async def test_list_sessions_per_page_zero(self, mock_session_manager, mock_current_user):
        from app.routes.sessions import list_sessions
...
class TestListSessionsRoute:
    def test_list_sessions_success(self, client, mock_manager):
    ...
    def test_list_sessions_invalid_per_page(self, client):
        resp = client.get("/v1/sessions?per_page=0")
        assert resp.status_code == 400

    def test_list_sessions_per_page_too_large(self, client):
        resp = client.get("/v1/sessions?per_page=101")
        assert resp.status_code == 400

    def test_list_sessions_500(self, client, mock_manager):
...
class TestCreateSessionRoute:
    def _setup_manager(self, mock_manager, mock_redis):
    ...
    def test_create_session_success(self, client, mock_manager, mock_redis):
    ...
    def test_create_session_no_idempotency_key(self, client, mock_manager, mock_redis):
    ...
    def test_create_session_with_idempotency_key_covers_cache_line(
        self, client, mock_manager, mock_redis
    ):
        """Sending an idempotency key exercises the cache_idempotency_response call (line 354)."""
        self._setup_manager(mock_manager, mock_redis)
        with patch(
    ...
    def test_create_session_idempotency_cached(self, client, mock_manager, mock_redis):
    ...
    def test_create_session_idempotency_key_too_long(self, client, mock_manager, mock_redis):
        mock_redis.get = AsyncMock(return_value=None)
        resp = client.post(
            "/v1/sessions",
            json={"config_path": "config/default.yaml"},
...
class TestGetSessionRoute:
    ...
    def test_get_session_success(self, client, mock_manager, mock_db):
    ...
    def test_get_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.get("/v1/sessions/nonexistent")
        assert resp.status_code == 404

    def test_get_session_not_found_in_manager(self, client, mock_manager, mock_db):
        self._setup_db(mock_db)
        mock_manager.get_session = AsyncMock(side_effect=ValueError("not found"))
        resp = client.get("/v1/sessions/sess-1")
        assert resp.status_code == 404

    def test_get_session_forbidden(self, client, mock_db):
...
class TestGetSessionMetricsRoute:
    ...
    def test_get_metrics_success(self, client, mock_manager, mock_db):
        self._setup_db(mock_db)
        sim_session = _make_sim_session()
    ...
    def test_get_metrics_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.get("/v1/sessions/nonexistent/metrics")
        assert resp.status_code == 404

    def test_get_metrics_session_not_found_in_manager(self, client, mock_manager, mock_db):
        self._setup_db(mock_db)
        mock_manager.get_session = AsyncMock(side_effect=ValueError("not found"))
        resp = client.get("/v1/sessions/sess-1/metrics")
        assert resp.status_code == 404

    def test_get_metrics_empty_state(self, client, mock_manager, mock_db):
...
class TestGetSessionTasksRoute:
    def _setup_db_with_tasks(self, mock_db, user_id="user-123"):
    ...
    def test_get_tasks_success(self, client, mock_db):
        session_model = _make_session_model()
        task = MagicMock()
        task.task_id = "task-1"
        task.status = "SUCCESS"
    ...
    def test_get_tasks_session_not_found(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.first.return_value = None
        resp = client.get("/v1/sessions/nonexistent/tasks")
        assert resp.status_code == 404

    def test_get_tasks_forbidden(self, client, mock_db):
    ...
    def test_get_tasks_with_error_message(self, client, mock_db):
...
class TestStartSessionRoute:
    def _setup(self, mock_db, mock_manager, state_val="running"):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_start_session_success(self, client, mock_manager, mock_db):
        self._setup(mock_db, mock_manager)
        resp = client.post("/v1/sessions/sess-1/start")
        assert resp.status_code == 200
        assert resp.json()["session_id"] == "sess-1"

    def test_start_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.post("/v1/sessions/nonexistent/start")
        assert resp.status_code == 404

    def test_start_session_not_found_in_manager(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_start_session_state_conflict(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
...
class TestPauseSessionRoute:
    def _setup(self, mock_db, mock_manager):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_pause_session_success(self, client, mock_manager, mock_db):
        self._setup(mock_db, mock_manager)
        resp = client.post("/v1/sessions/sess-1/pause")
        assert resp.status_code == 200

    def test_pause_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.post("/v1/sessions/nonexistent/pause")
        assert resp.status_code == 404

    def test_pause_session_not_found_in_manager(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_pause_session_state_conflict(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
...
class TestStopSessionRoute:
    ...
    def test_stop_session_success(self, client, mock_manager, mock_db):
        self._setup(mock_db, mock_manager)
        resp = client.post("/v1/sessions/sess-1/stop")
        assert resp.status_code == 200

    def test_stop_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.post("/v1/sessions/nonexistent/stop")
        assert resp.status_code == 404

    def test_stop_session_not_found_in_manager(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_stop_session_state_conflict(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
...
class TestResetSessionRoute:
    ...
    def test_reset_session_success(self, client, mock_manager, mock_db):
        self._setup(mock_db, mock_manager)
        resp = client.post("/v1/sessions/sess-1/reset")
        assert resp.status_code == 200

    def test_reset_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.post("/v1/sessions/nonexistent/reset")
        assert resp.status_code == 404

    def test_reset_session_not_found_in_manager(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_reset_session_state_conflict(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
...
class TestDeleteSessionRoute:
    ...
    def test_delete_session_success(self, client, mock_manager, mock_db):
        self._setup(mock_db, mock_manager)
        resp = client.delete("/v1/sessions/sess-1")
        assert resp.status_code == 204

    def test_delete_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.delete("/v1/sessions/nonexistent")
        assert resp.status_code == 404

    def test_delete_session_not_found_in_manager(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
...
class TestStepSessionRoute:
    ...
    def test_step_session_success(self, client, mock_manager, mock_db):
        self._setup(mock_db, mock_manager)
        resp = client.post("/v1/sessions/sess-1/step")
        assert resp.status_code == 200
        assert resp.json()["status"] == "stepped"

    def test_step_session_not_found_in_db(self, client, mock_db):
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = None
        resp = client.post("/v1/sessions/nonexistent/step")
        assert resp.status_code == 404

    def test_step_session_not_found_in_manager(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
    ...
    def test_step_session_invalid_state(self, client, mock_manager, mock_db):
        session_model = _make_session_model()
        mock_db.query.return_value.filter.return_value.filter.return_value.first.return_value = (
            session_model
        )
...
class TestSessionRoutesActions:
    ...
    def _setup_db(self, mock_db, user_id="user123"):
        session_model = Mock(spec=SessionModel)
        session_model.session_id = "session123"
