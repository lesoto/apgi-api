...
@pytest.fixture
def service(mock_redis: AsyncMock) -> CacheService:
...
class TestInit:
    def test_stores_redis_and_ttl(self, mock_redis: AsyncMock) -> None:
        svc = CacheService(mock_redis, default_ttl=1800)
        assert svc.redis is mock_redis
        assert svc.default_ttl == 1800

    def test_default_ttl_is_3600(self, mock_redis: AsyncMock) -> None:
        svc = CacheService(mock_redis)
        assert svc.default_ttl == 3600

    def test_prefixes_defined(self, service: CacheService) -> None:
    ...
    def test_fernet_created(self, service: CacheService) -> None:
...
class TestGlobalHelpers:
    def test_init_cache_service_sets_global(self, mock_redis: AsyncMock) -> None:
    ...
    def test_get_cache_service_none_when_not_init(self) -> None:
        import app.services.cache_service as mod

        mod.cache_service = None
        assert get_cache_service() is None

    def test_get_cache_service_returns_instance(self, mock_redis: AsyncMock) -> None:
...
class TestSetGetJson:
    @pytest.mark.asyncio
    async def test_set_json_calls_setex(self, service: CacheService, mock_redis: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_json_returns_none_on_exception(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
...
class TestEncryptedJson:
    @pytest.mark.asyncio
    async def test_set_encrypted_json_encrypts_and_stores(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
        mock_redis.setex = AsyncMock(return_value=True)
    ...
    @pytest.mark.asyncio
    async def test_set_encrypted_json_returns_false_on_error(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_encrypted_json_round_trip(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_encrypted_json_returns_none_when_missing(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
        mock_redis.get = AsyncMock(return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_get_encrypted_json_returns_none_on_bad_data(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
...
class TestSessionState:
    @pytest.mark.asyncio
    async def test_set_session_state_uses_correct_key(self, service: CacheService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_session_state_default_ttl(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_get_session_state_uses_correct_key(self, service: CacheService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_session_state_returns_none_when_missing(self, service: CacheService) -> None:
...
class TestSessionMetadata:
    @pytest.mark.asyncio
    async def test_set_session_metadata_uses_correct_key(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_get_session_metadata_uses_correct_key(self, service: CacheService) -> None:
...
class TestUserData:
    @pytest.mark.asyncio
    async def test_set_user_data_uses_encrypted_json(self, service: CacheService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_user_data_uses_encrypted_json(self, service: CacheService) -> None:
        with patch.object(
...
class TestAuthToken:
    @pytest.mark.asyncio
    async def test_set_auth_token_uses_correct_key(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
            await service.set_auth_token("hash123", {"uid": "u1"}, ttl=3600)
            m.assert_awaited_once_with("auth:hash123", {"uid": "u1"}, 3600)

    @pytest.mark.asyncio
    async def test_get_auth_token_uses_correct_key(self, service: CacheService) -> None:
...
class TestTaskResult:
    @pytest.mark.asyncio
    async def test_set_task_result_uses_correct_key(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_get_task_result_uses_correct_key(self, service: CacheService) -> None:
...
class TestQueryResult:
    @pytest.mark.asyncio
    async def test_set_query_result_default_ttl_is_quarter(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_set_query_result_custom_ttl(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_get_query_result_uses_correct_key(self, service: CacheService) -> None:
...
class TestApiResponse:
    @pytest.mark.asyncio
    async def test_set_api_response_default_ttl_is_half(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_set_api_response_key_format(self, service: CacheService) -> None:
        with patch.object(service, "_set_json", new=AsyncMock(return_value=True)) as m:
    ...
    @pytest.mark.asyncio
    async def test_get_api_response_uses_correct_key(self, service: CacheService) -> None:
...
class TestCacheInvalidation:
    @pytest.mark.asyncio
    async def test_invalidate_session_cache_deletes_matching_keys(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_invalidate_user_cache_deletes_matching_keys(
        self, service: CacheService, mock_redis: AsyncMock
    ) -> None:
        keys = [b"user:u1:data"]
    ...
    @pytest.mark.asyncio
    async def test_clear_expired_cache_returns_0(self, service: CacheService) -> None:
